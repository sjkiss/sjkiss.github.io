...
# Subject (topical) tags come from Zotero *Tags* (bib `keywords`), minus a set
# of administrative tags, unioned with any legacy `annotation` terms.
admin_tags <- c(
...
norm_term <- function(x) trimws(gsub("[-_[:space:]]+", " ", tolower(as.character(x))))
pretty_tag <- function(x) gsub("\\b([[:alpha:]])", "\\U\\1", gsub("[-_]+", " ", as.character(x)), perl = TRUE)
...
# Get reports
#bib_reports  <- ReadBib("publications/kiss_reports.bib", check = FALSE)

# combine
#bib<-c(bib_articles, bib_reports)
#  unlist(., use.names = T) ->bib

#bib  <- ReadBib("publications/kiss_reports.bib", check = FALSE)
keys <- names(bib)
# Only the author's own work (Zotero tag "mine") is listed.
keys <- keys[vapply(keys, function(k) is_mine(bib[k]), logical(1))]
...
build_card <- function(
  ...
  # Subject tags (from Zotero Tags / keywords, minus administrative tags)
  category_block <- NULL
  category_data <- ""
  subs <- subject_tags(entry)
  if (length(subs)) {
    category_tags <- lapply(subs, function(category) {
      tags$span(class = "category", pretty_tag(category))
    })
    category_block <- tags$div(class = "pub-category", category_tags)
    category_data <- paste(gsub(" ", "-", norm_term(subs)), collapse = ",")
  }
