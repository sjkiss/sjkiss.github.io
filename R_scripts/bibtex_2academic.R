#This code is taken from https://jc-castillo.com/blog/posts/quarto-migration/#how-to-manage-your-publications
# and https://raw.githubusercontent.com/juancarloscastillo/jc-quarto/refs/heads/main/publication/posts/bibtex_2academic_plus.R
# The function: bibtex_2academic
# It creates QMD YAML filds for each publication from a bib file
#bibfile<-"publications/articles/kiss_my_articles.bib"
...
#Eliminate Curly Braces
  mypubs %>% 
    mutate(title=str_remove_all(title, "\\{|\\}")) ->mypubs
  #Eliminate Curly Braces
  mypubs %>% 
    mutate(institution=str_remove_all(institution, "\\{|\\}")) ->mypubs
    mypubs <- mypubs  %>%
...
#Had to change this code because previously it used replace_na but that failed
  # when all entry types were missing because it was a logical vector
  
  mypubs<- mypubs %>% 
    mutate(abstract=case_when(
      is.na(abstract)~"Abstract not available",
      TRUE~abstract
    ))
